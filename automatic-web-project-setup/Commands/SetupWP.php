	...
	class SetupWP extends Command
	{
		...
		public function configure ()
		{
			$this->setName('setup-wp')
				->setDescription("Download the latest WordPress file, update hosts and vhosts file. Developed by Ali Shan")
		...
		public function execute (InputInterface $input, OutputInterface $output)
		{
			...
			$zipFileName = $this->makeFileName();
			
			
			
			if (! $this->vhosts_file_path){
				$helper = $this->getHelper('question');
				$question = new Question('Please enter vHosts file path [] : ', '');
				$this->vhosts_file_path = $helper->ask($input, $output, $question);
			}
			/*
			
			<VirtualHost *:80>
				ServerAdmin webmaster@presstigers.com
				DocumentRoot "E:\XAMPP\htdocs\client.dev"
			...
			$vhost_entry_elem .= "\n<VirtualHost *:80>";
			$vhost_entry_elem .= "\n\tServerAdmin webmaster@presstigers.com";
			$vhost_entry_elem .= "\n\tDocumentRoot \"webmaster@presstigers.com\"";
			$vhost_entry_elem .= "\n\tServerName  client.dev";
			$vhost_entry_elem .= "\n\tErrorLog  \"logs/dummy-host2.example.com-error.log\"";
			$vhost_entry_elem .= "\n\tErrorLog  \"logs/dummy-host2.example.com-access.log\" common";
			$vhost_entry_elem .= "\n----------";
			
			exec("echo $vhost_entry_elem >> " . $this->vhosts_file_path);
			exit(1);
			...
			$this->getDomainName($input, $output)
			...
			$this->assertDomainNotExistInVHostsFile($output)
				->download_wp($zipFileName, $output)
				->extract_zip($zipFileName, $output)
				->move_files($output)
				->updatingHostsFile($output);
			$this->updatingVHostsFile($output)
		...
		private function download_wp ($zipFileName, OutputInterface $output)
		{
			...
			$response = $this->client->get("//wordpress.org/latest.zip")->getBody();
